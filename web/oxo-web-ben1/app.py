...
def get_jira_client():
    ...
    if active_config is None:
        # Fallback to environment variables if no active config
        try:
...
def settings():
    """Jira settings page"""
    active_config = models.JiraConfiguration.query.filter_by(is_active=True).first()
    all_configs = models.JiraConfiguration.query.order_by(models.JiraConfiguration.created_at.desc()).all()
...
def create_jira_config():
    ...
    try:
        ...
        if data.get('is_active', False):
            models.JiraConfiguration.query.update({models.JiraConfiguration.is_active: False})
